...
const URL_CLAIM = 'https://www.playstation.com/en-us/ps-plus/whats-new/#monthly-games';

console.log(datetime(), 'started checking playstation plus');

const db = await jsonDb('psplus.json');
db.data ||= {};
...
try {
  ...
  // TODO: implement login and save user in db
  ...
  const monthlyGames = [... await monthlyGamesBlock.locator('.box')];
  
  const monthlyGamesNames = monthlyGames.map(async (el) => (await el.locator('.txt-style-medium-title .txt-block-paragraph__title')).innerText);
  // TODO iterate over game names to see if already claimed by checking db?

  const monthlyGamesPageLinks = monthlyGames.map(async (el) => (await el.locator('.cta__primary')).href);
  
  monthlyGamesPageLinks.forEach((gamePageLink) => {
    await page.goto(gamePageLink);

    // TODO make sure logged in since using subdomain (store.playstation.com)

    const gameCard = [...await page.locator('.psw-c-bg-card-1')][0];
    // const addToLibraryBtn = [...await gameCard.locator('.psw-purchase-button')][0];
    await gameCard.click('.psw-purchase-button', { delay: 11 });

    // TODO wait for "Added to library" and/or "Download from Library"
    // TODO update db.data with claimed game
  })
